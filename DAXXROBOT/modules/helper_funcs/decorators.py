...
from DAXXROBOT import dispatcher as n
from DAXXROBOT.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler
...
class MukeshTelegramHandler:
    ...
    def message(
        ...
        def _message(func):
            try:
                ...
                LOGGER.debug(
                    f"[DAXXROBOT-Msg] Loaded filter pattern {pattern} for function {func.__name__}"
                )
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            LOGGER.debug(
                f"[DAXXROBOT-Callback] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}"
            )
            return func
